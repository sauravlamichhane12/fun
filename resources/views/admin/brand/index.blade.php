...
<div class="row">
    ...
    <button type="button" class="btn btn-default" style="" disabled>
                <h5>List of brands information</h5></button>

               <a href="{{ route('brand.create') }}"><button type="button" class="btn btn-primary" style="float:right">
                 Add Brand</button></a>
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <td>{{ $key++ }}</td>
                    ...
                    <form method="post" action="{{ url('admin/brand/status',$brand->id) }}">                     
                       @csrf
                        @if($brand->status==1)
                        <td>
                    ...
                    <td><a href="{{ route('brand.edit',$brand->id) }}"><button type="button" class="btn btn-info btn-sm">
                    <i class="fas fa-pen"></i></button></a>

                    <button type="button" class="btn btn-danger btn-sm"  data-toggle="modal" data-target="#modal-default" onclick="handleDelete({{ $brand->id  }})">
